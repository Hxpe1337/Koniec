...
function setupChart() {
    ...
    updateChart(stockChart, stockPrices);
}

let zoomLevel = 20; // Startowy poziom zoomu
function updateChart(stockChart, stockPrices) {
    setInterval(() => {
        ...
        stockChart.data.labels.push(''); // Dodaj pustą etykietę
        stockChart.data.datasets[0].data = stockPrices.slice(-zoomLevel);
        stockChart.update();
    }, 1000);
}
